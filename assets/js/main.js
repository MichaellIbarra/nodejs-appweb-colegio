...
(function ($) {
  ...
  $('.cni-menus li a').each(function () {
    if (this.href === path) {
      $(this).addClass('cni-current-page');
    }
  });

  // Sticky Menu
 // Obtener el elemento con clase "cni-header"
var header = document.querySelector('.cni-header');

// Agregar un evento de desplazamiento a la ventana (window)
window.addEventListener('scroll', function () {
    // Obtener la posición de desplazamiento vertical
    var scroll = window.scrollY || document.documentElement.scrollTop;

    // Verificar si la posición de desplazamiento es menor que 100 píxeles
    if (scroll < 100) {
        // Eliminar la clase "cni-sticky" del encabezado
        header.classList.remove('cni-sticky');
    } else {
        // Agregar la clase "cni-sticky" al encabezado
  ...
  // Client Slider Part
  if ($('.feedcni-slider').length) {
    $('.feedcni-slider').owlCarousel({
      loop: true,
      items: 3,
      margin: 25,
      autoplay: false,
  ...
  // Client Slider Part
  if ($('#cni-blog-slider').length) {
    $('#cni-blog-slider').owlCarousel({
      loop: true,
      items: 3,
      margin: 20,
      autoplay: false,
  ...
  //filter js
  var pifilter = $('.cni-grid');
  if (pifilter.length) {
    $('.cni-grid').imagesLoaded(function () {
      $('.cni-filter').on('click', 'button', function () {
        var filterValue = $(this).attr('data-filter');
        $grid.isotope({
          filter: filterValue
        });
      });
      var $grid = $('.cni-grid').isotope({
  ...
  // portfolio Filter
  var filterbutton = $('.cni-filter button');
  if (filterbutton.length) {
    $('.cni-filter button').on('click', function (event) {
  ...
  $(window).scroll(function (event) {
    ...
    if (st > lastScrollTop) {
      $("#backscrollUp").removeClass("cni__up___scroll");
    } else {
      $("#backscrollUp").addClass("cni__up___scroll");
    }
    lastScrollTop = st;
  });
